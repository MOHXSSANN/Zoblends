...
void main(){
  ...
  float lD=dp+(1.-dp)*u_liquid*tb;lD+=noiseVal*u_distort*.15*edgeFactor;float rB=clamp(1.-cv,0.,1.);
  float fl=ag.x+sl;fl+=noiseVal*sl*u_distort*edgeFactor;fl*=mix(1.,1.-dp*.5,u_contour);fl-=dp*u_contour*.8;
  ...
  col=mix(col,1.-min(vec3(1.),(1.-col)/max(u_tint,vec3(.001))),length(u_tint-1.)*.5);col=clamp(col,0.,1.);
  oC=vec4(col*vs,vs);
}`;
...
function processImage(img: HTMLImageElement) {
  ...
  for (let i = 0; i < size; i++) {
    const idx = i * 4, r = data[idx], g = data[idx+1], b = data[idx+2], a = data[idx+3];
    const isBg = (r > 250 && g > 250 && b > 250 && a === 255) || a < 5;
    alphaValues[i] = isBg ? 0 : a / 255;
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (x===0||x===width-1||y===0||y===height-1||!shapeMask[idx-1]||!shapeMask[idx+1]||!shapeMask[idx-width]||!shapeMask[idx+width]) boundaryMask[idx] = 1;
    }
  }
  ...
  for (let iter = 0; iter < ITERATIONS; iter++) {
    for (let y = 1; y < height-1; y++) {
      for (let x = 1; x < width-1; x++) {
        ...
        const sum = (shapeMask[idx+1]?u[idx+1]:0)+(shapeMask[idx-1]?u[idx-1]:0)+(shapeMask[idx+width]?u[idx+width]:0)+(shapeMask[idx-width]?u[idx-width]:0);
  ...
  for (let i = 0; i < size; i++) {
    const px = i * 4, depth = u[i] / maxVal, gray = Math.round(255 * (1 - depth * depth));
...
export default function MetallicPaint({
  imageSrc, seed=42, scale=4, refraction=0.01, blur=0.015,
  ...
  const initGL = useCallback(() => {
    ...
    for (let i = 0; i < count; i++) { const info = gl.getActiveUniform(prog, i); if (info) uniforms[info.name] = gl.getUniformLocation(prog, info.name); }
    ...
    glRef.current = gl; uniformsRef.current = uniforms;
    return true;
  }, []);

  const uploadTexture = useCallback((imgData: ImageData) => {
    const gl = glRef.current; const u = uniformsRef.current;
    if (!gl) return;
  ...
  useEffect(() => {
    const gl = glRef.current; const u = uniformsRef.current;
    if (!gl || !ready) return;
    gl.uniform1f(u.u_seed, seed); gl.uniform1f(u.u_scale, scale);
    gl.uniform1f(u.u_refract, refraction); gl.uniform1f(u.u_blur, blur);
    gl.uniform1f(u.u_liquid, liquid); gl.uniform1f(u.u_bright, brightness);
    gl.uniform1f(u.u_contrast, contrast); gl.uniform1f(u.u_angle, angle);
    gl.uniform1f(u.u_fresnel, fresnel); gl.uniform1f(u.u_sharp, patternSharpness);
    gl.uniform1f(u.u_wave, waveAmplitude); gl.uniform1f(u.u_noise, noiseScale);
    gl.uniform1f(u.u_chroma, chromaticSpread); gl.uniform1f(u.u_distort, distortion);
  ...
  useEffect(() => {
    ...
    const gl = glRef.current!; const u = uniformsRef.current;
    const canvas = canvasRef.current!; const mouse = mouseRef.current;
    ...
    rafRef.current = requestAnimationFrame(render);
    return () => { if (rafRef.current) cancelAnimationFrame(rafRef.current); canvas.removeEventListener('mousemove', onMove); };
  }, [ready, textureReady]);
